'''
This file is the entrance of our whole model
'''
import argparse
import torch
import torchvision
from examples.simple.model.main import adjust_learning_rate, path_list
from examples.simple.model.our_model import Classifier, ExtractMacula, ExtractOptic
...
def main(args):
    ...
    if "efficientnet" in args.model_detail:
        model_extract_macular = ExtractMacula()
        model_extract_optic = ExtractOptic()
        '''
        Users need to add the codes to load your trained model for extraction locally
        '''
        # we define the binocular model
        classifier = Classifier(args, model_extract_macular, model_extract_optic).to(device)
    else:
        classifier = model_dict[args.model_detail].to(device)
    print(classifier)
    # we define the trainer to train our binocular model or baselines
